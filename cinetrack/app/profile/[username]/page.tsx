...
export default async function ProfilePage({
  ...
  const moviesWatchedThisYear = watched.filter(
    (w) => w.watched_at && new Date(w.watched_at).getFullYear() === thisYear
  ).length
  const showsCompleted = progress.filter((p) => p.status === 'completed').length

  const genreCounts = new Map<string, number>()
  for (const w of watched) {
    for (const g of w.movies?.genres ?? []) {
      genreCounts.set(g, (genreCounts.get(g) ?? 0) + 1)
    }
  }
  for (const p of progress) {
    for (const g of p.tv_shows?.genres ?? []) {
      genreCounts.set(g, (genreCounts.get(g) ?? 0) + 1)
    }
  }
  ...
  const allRatings = [
    ...watched.map((w) => w.rating),
    ...progress.map((p) => p.rating),
      ...
      <div className="mb-8 flex flex-wrap items-start justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            {profile.bio && (
              <p className="max-w-md text-sm text-ink/70 dark:text-paper/70">{profile.bio}</p>
            )}
      ...
      {topGenres.length > 0 && (
        <div className="mb-10 flex flex-wrap gap-2">
          {topGenres.map(([genre, count]) => (
            <span
              key={genre}
              className="rounded-full bg-ink/5 px-3 py-1 text-xs dark:bg-paper/10"
            >
              {genre}{' '}
              <span className="font-mono text-muted dark:text-mutedDark">({count})</span>
            </span>
          ))}
        </div>
      )}
          ...
          <div className="space-y-3">
            {reviews.map((r) => {
              ...
              return (
                <div
                  key={r.id}
                  className="rounded-xl border border-line p-4 text-sm dark:border-lineDark"
                >
                  <div className="mb-1 flex flex-wrap items-center gap-2">
                    <Link href={href} className="font-display font-medium hover:text-marquee">
                      {title}
                    </Link>
                    <span className="font-mono text-xs text-muted dark:text-mutedDark">
                      {r.rating}/10
                    </span>
                    {r.has_spoilers && (
